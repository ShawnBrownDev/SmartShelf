...
BEGIN
    ...
    user_last_name := COALESCE(NEW.raw_user_meta_data->>'lastName', 'Name');
    
    -- Ensure username meets constraints (length >= 3, alphanumeric + underscore only)
    ...
    IF char_length(trim(user_first_name)) = 0 THEN
        user_first_name := 'User';
    END IF;
    
    IF char_length(trim(user_last_name)) = 0 THEN
        user_last_name := 'Name';
    END IF;
    
    -- Create user profile with validated fields
...
EXCEPTION
    WHEN unique_violation THEN
        ...
        user_username := 'user_' || substr(NEW.id::text, 1, 8) || '_' || extract(epoch from now())::bigint % 1000;
